...
impl OAuth {
	...
	async fn get_auth_explicit_flow(&mut self, code: String) -> core::result::Result<(), Error> {
		...
		let auth_response: AuthResponse = serde_json::from_str(&response)?;
	...
	pub async fn refresh_auth(&mut self) -> core::result::Result<(), Error> {
		let mut params = HashMap::new();
	...
	pub async fn get_wcif_api(&self, id: &str) -> core::result::Result<String, Error> {
		let get_url = format!(
	...
	pub async fn get_wcif(&self, id: &str) -> core::result::Result<WcifContainer, Error> {
		let json = self.get_wcif_api(id).await?;
		parse(json)
	}

	pub async fn get_competitions_managed_by_me(
		&self,
	) -> core::result::Result<Vec<Competition>, Error> {
	...
	async fn patch_wcif(&self, wcif: &Wcif, id: &str) -> core::result::Result<String, Error> {
		let patch_url = format!(
			"https://api.worldcubeassociation.org/competitions/{}/wcif",
			id
		);

		let json = serde_json::to_string(wcif)?;
...
impl WcifContainer {
	...
	pub fn get_mut(&mut self) -> &mut Wcif {
		&mut self.wcif
	}

	pub fn get(&self) -> &Wcif {
		&self.wcif
	}

	pub async fn patch(&self, oauth: &OAuth) -> core::result::Result<String, Error> {
	...
	pub fn overlapping_activities(&self) -> Vec<(&Activity, &Activity)> {
	...
	pub fn add_groups_to_event(
		...
		self.wcif
			...
			.map(|a| {
				if !a.child_activities.is_empty() {
