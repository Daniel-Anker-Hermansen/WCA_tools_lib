...
pub fn draw_scorecard(
	generator: &mut ScorecardGenerator,
	scorecard: Scorecard,
	map: &HashMap<u32, &str>,
	limits: &HashMap<&str, TimeLimit>,
) {
	let language = &Language::english();
	let event = scorecard.event();
	...
	let attempts_amount = match scorecard.event().as_str() {
	...
	generator.write(
		&language.scram,
		9.0 + sign_box_width / 2.0,
		attempts_start_height - 1.0,
		7.0,
		Center,
		Normal,
	);
	...
	generator.write(
		&language.judge,
		100.0 - sign_box_width - (sign_box_width / 2.0),
		attempts_start_height - 1.0,
		7.0,
		Center,
		Normal,
	...
	for i in 0..attempts_amount {
		...
		generator.draw_square(
			9.0,
			attempts_start_height + j * distance,
			sign_box_width,
			height,
		);
		generator.write(
			...
			5.0,
			attempts_start_height - 2.0 + j * distance + height,
			12.0,
			Left,
			Normal,
		);
		generator.draw_square(
			9.0 + sign_box_width,
			attempts_start_height + j * distance,
			91.0 - 3.0 * sign_box_width,
			height,
		);
		generator.draw_square(
			100.0 - 2.0 * sign_box_width,
			attempts_start_height + j * distance,
			sign_box_width,
			height,
		);
		generator.draw_square(
			100.0 - sign_box_width,
			attempts_start_height + j * distance,
			sign_box_width,
			height,
		);
	...
	let limit = match scorecard.limit(limits) {
		...
		TimeLimit::SharedCumulative(z, vec) => format!(
			"{}: {} {} {}",
			language.cumulative_limit,
			time_string(*z),
			language.for_scl,
			vec.iter()
				.map(|x| get_event(x))
	...
	generator.write(&scorecard.station(), 100.0, 12.0, 20.0, Right, Bold);
}
...
fn get_event_func<'a>(language: &'a Language) -> impl Fn(&'a str) -> &'a str {
	|x| match x {
