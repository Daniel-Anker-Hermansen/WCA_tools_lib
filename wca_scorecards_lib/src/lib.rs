...
use scorecard_to_pdf::{Language, Return, Scorecard};
...
#[allow(deprecated)]
#[deprecated]
pub fn print_round_1<I>(args: &mut I)
where
...
#[deprecated]
pub fn print_round_1_with_language<I>(args: &mut I, language: Language)
where
	I: Iterator<Item = String>,
{
	...
	run(
		...
		&c,
		language,
		Stages::new(1, u32::MAX, false),
		ScorecardOrdering::Default,
	);
}

pub fn print_round_1_english(
	groups_csv: &str,
...
pub fn blank_scorecard_page(competition: &str) {
	save_pdf(
		scorecard_to_pdf::blank_scorecard_page(competition, &Language::english()),
		competition,
		"blank_",
	)
	.unwrap();
...
pub fn round_1_scorecards_in_memory_for_python(
	groups_csv: String,
	limit_csv: Option<String>,
	competition: &str,
	no_stages: u32,
