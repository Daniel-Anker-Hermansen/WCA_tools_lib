...
pub fn save_pdf(data: Return, competition: &str, prefix: &str) -> std::io::Result<()> {
	...
	let file_name = format!(
		"{prefix}{}_scorecards{name}",
		competition.split_ascii_whitespace().collect::<String>()
	);
		...
		Some(limit_csv) => {
			...
			limit_data.zip(event_list).for_each(|(x, event)| {
				...
				match v {
					...
					"K" => limits.insert(
						event,
						TimeLimit::Cutoff(usize_from_iter(&mut iter), usize_from_iter(&mut iter)),
					),
					"S" => limits.insert(
						event,
...
pub(crate) fn run_from_wcif(
	...
	get_round_json(wcif, event, round)
		.unwrap()
		.scramble_set_count = groups.len();
	let mut limits = HashMap::new();
	...
	let mut k = groups
		...
		.zip(1..)
		.map(|(group, n)| {
			group.into_iter().map(move |(id, station)| Scorecard {
				event,
				round,
				group: if stages.seperate_stages {
					(n - 1) * stages.no as usize + 1 + station / stages.capacity as usize
				} else {
					n
				},
				station: Some(if stages.seperate_stages {
					(station - 1) % stages.capacity as usize + 1
				} else {
					station
				}),
				id: Some(id),
...
fn usize_from_iter<'a, I>(iter: &mut I) -> usize
where
	I: Iterator<Item = &'a str>,
{
	match usize::from_str_radix(
		...
		Err(_) => panic!(
			"Malformatted input file. Expected positive integer, but received other charachters"
		),
		Ok(v) => v,
	}
}
...
pub(crate) fn blank_for_subsequent(wcif: &Wcif, stations: usize) -> Return {
	...
	scorecard_to_pdf::scorecards_to_pdf(
		scorecards,
		name,
		&HashMap::new(),
		&HashMap::new(),
